...
# ═══════════════════════════════════════════════════════
#  DATABASE PATH  (one and only one definition)
# ═══════════════════════════════════════════════════════

def _db_path():
    """Writable DB path on Android and desktop."""
    try:
...
def setup_db():
    """Create tables and seed demo data (safe to call many times)."""
    with db() as con:
        cur = con.cursor()
        ...
        if not cur.execute("SELECT 1 FROM products LIMIT 1").fetchone():
            cur.executemany(
                "INSERT INTO products(name,category,qty,cost,price,min_qty) VALUES(?,?,?,?,?,?)",
                [
...
<SplashScreen>:
    ...
    BoxLayout:
        ...
        MDLabel:
            ...
            theme_text_color: "Custom"
            text_color: 0.42, 0.22, 0.07, 0.65
        Widget:
...
<InventoryScreen>:
    ...
    BoxLayout:
        ...
        MDTopAppBar:
            ...
            left_action_items: [["arrow-left", lambda x: app.back()]]
            right_action_items: [["plus", lambda x: app.goto("add_product")]]
            md_bg_color: 0.42, 0.22, 0.07, 1
            specific_text_color: 1, 1, 1, 1
        ScrollView:
            MDList:
...
<CustomerScreen>:
    ...
    BoxLayout:
        ...
        MDTopAppBar:
            title: "Customers"
            left_action_items: [["arrow-left", lambda x: app.back()]]
            right_action_items: [["plus", lambda x: app.add_customer_dialog()]]
            md_bg_color: 0.42, 0.22, 0.07, 1
            specific_text_color: 1, 1, 1, 1
        ScrollView:
            MDList:
...
class JebenaApp(MDApp):
    ...
    def back(self):
        if Session.owner():
            self._load_owner()
            self._show("owner", direction="right")
        else:
            self._load_staff()
            self._show("staff", direction="right")
    ...
    def _load_owner(self):
        ...
        sc.ids.greeting.text    = "Hello {}!".format(Session.name)
    ...
    def _load_staff(self):
        ...
        sc.ids.staff_greeting.text = "Hello {}!".format(Session.name)
        sc.ids.staff_income.text   = "Today: {:,.0f} Birr".format(income)
        sc.ids.staff_count.text    = "{} items sold".format(count)
    ...
    def open_product_menu(self, btn):
        ...
        self._prod_menu = MDDropdownMenu(caller=btn, items=items, width_mult=5)
        self._prod_menu.open()

    def _pick_product(self, row):
        sc = self.sm.get_screen("sale")
        sc._product                  = row
        sc.ids.sale_product_btn.text = "{} ▼".format(row[1])
    ...
    def open_payment_menu(self, btn):
        ...
        self._pay_menu = MDDropdownMenu(caller=btn, items=items, width_mult=4)
        self._pay_menu.open()

    def _pick_payment(self, method):
        sc = self.sm.get_screen("sale")
        sc._payment              = method
        sc.ids.sale_pay_btn.text = "Payment: {} ▼".format(method)
    ...
    def calc_total(self, *args):
        ...
        try:
            ...
            sc.ids.sale_total_lbl.text = "Total: {:,.0f} Birr".format(qty * price)
        except Exception:
            pass
    ...
    def save_sale(self, *args):
        ...
        with db() as con:
            con.execute(
                "INSERT INTO sales"
                "(product_id,product_name,qty,unit_price,total,payment,staff_id,staff_name)"
                " VALUES (?,?,?,?,?,?,?,?)",
                (pid, pname, qty, price, total, sc._payment, Session.uid, Session.name),
            )
            con.execute("UPDATE products SET qty=qty-? WHERE id=?", (qty, pid))

        Snackbar(text="Saved!  {:,.0f} Birr".format(total)).open()
        self._reset_sale()
    ...
    def _load_inventory(self):
        ...
        for r in rows:
            ...
            lst.add_widget(TwoLineListItem(
                ...
                secondary_text="Stock: {}  |  Price: {:.0f} Birr".format(qty, price),
            ))
    ...
    def open_cat_menu(self, btn):
        ...
        self._cat_menu = MDDropdownMenu(caller=btn, items=items, width_mult=3)
        self._cat_menu.open()

    def _pick_cat(self, cat):
        sc = self.sm.get_screen("add_product")
        sc._cat = cat
        sc.ids.ap_cat_btn.text = "Category: {} ▼".format(cat)
    ...
    def save_product(self, *args):
        ...
        Snackbar(text="{} saved!".format(name)).open()
    ...
    def _load_report(self):
        ...
        with db() as con:
            ...
            expense = con.execute(
                "SELECT COALESCE(SUM(amount),0) FROM expenses WHERE date(created_at)=?",
                (today,)
            ).fetchone()[0]
            rows = con.execute(
                "SELECT product_name,qty,total,staff_name,payment "
                "FROM sales WHERE date(created_at)=? ORDER BY created_at DESC",
                (today,)
            ).fetchall()
        ...
        for r in rows:
            lst.add_widget(ThreeLineListItem(
                ...
                tertiary_text="Staff: {}".format(r[3]),
            ))
    ...
    def open_exp_cat_menu(self, btn):
        ...
        self._exp_cat_menu = MDDropdownMenu(caller=btn, items=items, width_mult=4)
        self._exp_cat_menu.open()

    def _pick_exp_cat(self, cat):
        sc = self.sm.get_screen("expense")
        sc._cat = cat
        sc.ids.exp_cat_btn.text = "Category: {} ▼".format(cat)
    ...
    def _save_customer(self, *args):
        ...
        with db() as con:
            con.execute(
                ...
                (name, self._dlg_phone.text.strip(), self._dlg_address.text.strip()),
            )
        self._dialog.dismiss()
        Snackbar(text="{} added!".format(name)).open()
